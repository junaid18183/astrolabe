...
const transformOneOfAnyOfAllOf = (
...
function generateCR(crd: CRD, specContent: CRDSpec) {
  const apiVersion = crd.spec.versions
    ? crd.spec.group + '/' + crd.spec.versions[0].name
    : crd.spec.group + '/V0';
  if (!crd) {
    return null;
  }
  // Use name/namespace from form data if present, else fallback
  const name = specContent.metadata?.name || 'new-custom-resource';
  const namespace = specContent.metadata?.namespace;
  const cr = {
...
const DynamicForm = ({ schema, crd, onSubmit }: DynamicFormProps) => {
  ...
  useEffect(() => {
    ...
    const jsonSchema = transformSchema(schema, ['resourceRef', 'resourceRefs']);
