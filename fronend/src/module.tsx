  ...
  return (
    <div>
      {modules.map((module, index) => (
        <div key={index}>
          {/* Example: Render the jsonData and _clusterName if they are strings or numbers */}
          <p>JSON Data: {JSON.stringify(module.jsonData)}</p>
          <p>Cluster Name: {module._clusterName}</p>
        </div>
      ))}
    </div>
  );
