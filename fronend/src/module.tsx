...
function ModuleListView() {
  ...
  return (
    <ResourceListView
      title="Astrolabe Modules"
      data={modules}
      columns={[
        {
          label: 'Name',
          getter: (m: any) => (
            <Link to={`/astrolabe/modules/${m.metadata.namespace}/${m.metadata.name}`}>
              {m.metadata.name}
            </Link>
          ),
        },
        {
          label: 'Namespace',
          getter: (m: any) => m.metadata.namespace,
        },
        {
          label: 'Source Type',
          getter: (m: any) => m.spec?.source?.type || '-',
        },
        {
          label: 'Source URL',
          getter: (m: any) => m.spec?.source?.url || '-',
        },
        {
          label: 'Version',
          getter: (m: any) => m.spec?.source?.version || '-',
        },
        {
          label: 'Ready',
          getter: (m: any) => {
            const readyCond = (m.status?.conditions || []).find((c: any) => c.type === 'Ready');
            return <StatusLabel status={readyCond?.status || '-'} />;
          },
        },
      ]}
    />
